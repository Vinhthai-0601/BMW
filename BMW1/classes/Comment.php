...
class Comment {
  ...
  public function outputComments() {
    ...
    foreach ($this->comments as $comment) {
      $output .= "<div class='card mt-2 mb-2'><div class='card-header'> {$comment['username']} | {$comment['date_created']} <a href='func/commentmanager.php?id={$comment['ID']}'><button type='button' class='btn delete-post btn-outline-danger btn-sm  float-right'>X</button></a></div><div class='card-body'><p class='card-text'>{$comment['comment_text']} </p></div></div>";
    }
    echo $output;
  }
